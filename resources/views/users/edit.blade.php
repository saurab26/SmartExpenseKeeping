		...
		<div class="col-sm-8 col-sm-offset-2">
			<form action="{{ route('user.update', $user->id) }}" class="form-horizontal" role="form" method="post">
				...
				<div class="form-group {{ $errors->has('post_code') ? 'has-error':'' }}">
					...
					<div class="col-sm-10">
						<input type="text" value="{{$user->post_code }}" class="form-control" name="post_code">
				...
				<div class="form-group" id="accessibilities" <?php echo $display; ?>>
					<label for="role" class="col-sm-2 form-control-label">Permissions</label>
					<div class="col-sm-10">

						@if(count($companies))
						@foreach($companies as $company)
						<label for=""><input  type="checkbox" value="{{ $company->id }}" name="access[{{ $company->id }}]" onclick="categories($(this),{{ $company->id }})"<?php if($users->exists($id,$company->id)){echo 'checked';} ?>>{{ $company->name }}</label><br>
						...
						<ul style="list-style: none;" id="checkbox_{{ $company->id }}">
							...
							<li><label><input type="{{ $checkbox }}" value="{{ $category->id }}" name="access[{{ $company->id }}][]" class="categories"<?php if($users->exists($id,$company->id,$category->id)){echo 'checked';}?>> {{ $category->name }}</label></li>
